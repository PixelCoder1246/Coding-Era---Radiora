...
def scan_mri(model_id, dcm_dir, body_part="brain"):
    ...
    # Process all `.dcm` or `.dicom` files
    dcm_files = [f for f in os.listdir(dcm_dir) if f.lower().endswith(".dcm") or f.lower().endswith(".dicom")]
    dcm_files.sort()  # Alphanumeric sort for z-axis sequencing
